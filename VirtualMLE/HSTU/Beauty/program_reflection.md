# program_reflection — HSTU / Beauty
...
Improve validation Recall@10 on `HSTU` for the `Beauty` domain while keeping the release code path simple, reproducible, and easy to audit.
...
- `../run_hstu.py` can resolve `--domain beauty` to the default dataset path.
...
```bash
mkdir -p output
python ../run_hstu.py   --domain beauty   --output_json output/run_result_<tag>.json   > output/run_<tag>.log 2>&1
```

If a concrete change is being tested, append only the required arguments in the same command, for example:

```bash
python ../run_hstu.py   --domain beauty   --epochs 20   --hidden_dim 128   --linear_dim 16   --attention_dim 16   --num_blocks 2   --num_heads 2   --dropout 0.3   --attn_dropout 0.1   --optimizer adamw   --lr 0.001   --weight_decay 0.0   --temperature 0.05   --output_json output/run_result_search_<tag>.json   > output/run_search_<tag>.log 2>&1
```
...
- Number of users: 22363
- Number of items: 12101
- Average full-sequence length: 8.71
- P50 / P90 / P95 sequence length: 6 / 14 / 21
