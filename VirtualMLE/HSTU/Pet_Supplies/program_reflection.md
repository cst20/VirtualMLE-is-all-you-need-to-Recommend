# program_reflection — HSTU / Pet_Supplies
...
Improve validation Recall@10 on `HSTU` for the `Pet_Supplies` domain while keeping the release code path simple, reproducible, and easy to audit.
...
- `../run_hstu.py` can resolve `--domain pet_supplies` to the default dataset path.
...
```bash
mkdir -p output
python ../run_hstu.py   --domain pet_supplies   --output_json output/run_result_<tag>.json   > output/run_<tag>.log 2>&1
```

If a concrete change is being tested, append only the required arguments in the same command, for example:

```bash
python ../run_hstu.py   --domain pet_supplies   --epochs 20   --hidden_dim 128   --linear_dim 16   --attention_dim 16   --num_blocks 2   --num_heads 2   --dropout 0.3   --attn_dropout 0.1   --optimizer adamw   --lr 0.001   --weight_decay 0.0   --temperature 0.05   --output_json output/run_result_search_<tag>.json   > output/run_search_<tag>.log 2>&1
```
...
- Number of users: 19856
- Number of items: 8510
- Average full-sequence length: 7.92
- P50 / P90 / P95 sequence length: 6 / 13 / 16
